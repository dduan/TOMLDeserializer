...
enum TOMLParser {
    ...
    static let decInt = decIntText.map { Int(String($0))! }

    static func int(_ prefix: FixedPrefix, _ digit: PredicateChar, radix: Int) -> AnyParser<Text, Int> {
        prefix
            .take(digit)
            .map { _, x in x }
            .take(
            ...
            .map { first, rest -> Int in
                Int(String([first] + rest), radix: radix)!
            }
            .eraseToAny()
    }
